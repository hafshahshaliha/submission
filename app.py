...
def create_rfm_df(df):
    rfm_df = df.groupby(by="customer_id", as_index=False).agg({
        "order_approved_date": "max", #mengambil tanggal order terakhir
        "order_id": "nunique",
        "price": "sum"
    })
    ...
    recent_date = df["order_approved_date"].dt.date.max()
...
all_df.sort_values(by="order_approved_date", inplace=True)
...
# Filter data
min_date = all_df["order_approved_date"].min()
max_date = all_df["order_approved_date"].max()

with st.sidebar:
    # Menambahkan logo perusahaan
    st.image("españa warehouse.png")
...
main_df = all_df[(all_df["order_approved_date"] >= str(start_date)) & 
                (all_df["order_approved_date"] <= str(end_date))]
...
st.header('En España Warehouse Dashboard:magic_wand:')
...
sns.barplot(y="recency", x="customer_id", 
            ...
            hue="customer_id",  # Menambahkan hue jika relevan
...
st.caption('Made by En España Warehouse 2024')